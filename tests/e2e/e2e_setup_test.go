...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	//
	// 1. Initialize Teritori validator nodes.
	// 2. Create and initialize Teritor validator genesis files (both chains)
...
func (s *IntegrationTestSuite) TearDownSuite() {
	if str := os.Getenv("TERITORI_E2E_SKIP_CLEANUP"); len(str) > 0 {
...
func (s *IntegrationTestSuite) runValidators(c *chain, portOffset int) {
	s.T().Logf("starting Teritor %s validator containers...", c.id)
	...
	for i, val := range c.validators {
		runOpts := &dockertest.RunOptions{
			...
			Mounts: []string{
				fmt.Sprintf("%s/:%s", val.configDir(), teritoriHomePath),
			},
			Cmd:        []string{teritoridBinary, "start"},
			Repository: "teritori/teritorid-e2e",
		}
		...
		s.T().Logf("started Teritori %s validator container: %s", c.id, resource.Container.ID)
	}
	...
	s.Require().Eventually(
		...
		5*time.Minute,
		time.Second,
		"Teritori node failed to produce blocks",
	)
}
...
func (s *IntegrationTestSuite) runIBCRelayer() {
	...
	tmpDir, err := os.MkdirTemp("", "teritori-e2e-testnet-hermes-")
	s.Require().NoError(err)
	s.tmpDirs = append(s.tmpDirs, tmpDir)

	teritoriAVal := s.chainA.validators[0]
	teritoriBVal := s.chainB.validators[0]

	teritoriARly := s.chainA.genesisAccounts[relayerAccountIndexHermes]
	teritoriBRly := s.chainB.genesisAccounts[relayerAccountIndexHermes]
	...
	s.hermesResource, err = s.dkrPool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("TERITORI_A_E2E_CHAIN_ID=%s", s.chainA.id),
				fmt.Sprintf("TERITORI_B_E2E_CHAIN_ID=%s", s.chainB.id),
				fmt.Sprintf("TERITORI_A_E2E_VAL_MNEMONIC=%s", teritoriAVal.mnemonic),
				fmt.Sprintf("TERITORI_B_E2E_VAL_MNEMONIC=%s", teritoriBVal.mnemonic),
				fmt.Sprintf("TERITORI_A_E2E_RLY_MNEMONIC=%s", teritoriARly.mnemonic),
				fmt.Sprintf("TERITORI_B_E2E_RLY_MNEMONIC=%s", teritoriBRly.mnemonic),
	...
	// create the client, connection and channel between the two teritori chains
	s.createConnection()
	s.createChannel()
}
