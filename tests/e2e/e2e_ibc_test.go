...
func (s *IntegrationTestSuite) sendIBC(c *chain, valIdx int, sender, recipient, token, fees, note string, expErr bool) {
	...
	if expErr {
		s.executeTeritoriTxCommand(ctx, c, ibcCmd, valIdx, s.expectErrExecValidation(c, valIdx, true))
		s.T().Log("unsuccessfully sent IBC tokens")
	} else {
		s.executeTeritoriTxCommand(ctx, c, ibcCmd, valIdx, s.defaultExecValidation(c, valIdx))
...
func (s *IntegrationTestSuite) testIBCTokenTransfer() {
	s.Run("send_utori_to_chainB", func() {
		...
		s.Require().Eventually(
			func() bool {
				balances, err = queryTeritoriAllBalances(chainBAPIEndpoint, recipient)
