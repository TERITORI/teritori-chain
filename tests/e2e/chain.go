...
var (
	encodingConfig teritoriparams.EncodingConfig
	cdc            codec.Codec
	txConfig       client.TxConfig
)

func init() {
	encodingConfig = teritoriparams.MakeEncodingConfig()
...
func newChain() (*chain, error) {
	tmpDir, err := os.MkdirTemp("", "teritori-e2e-testnet-")
	if err != nil {
		return nil, err
	}
...
func (c *chain) createValidator(index int) *validator {
	return &validator{
		chain:   c,
		index:   index,
		moniker: fmt.Sprintf("%s-teritori-%d", c.id, index),
	}
}
